...
export default function Card_NuestrosClientes() {
  const Cards = Data_CardNuestrosClientes.map((client) => (
    <motion.article
      ...
      className=" w-60  justify-center transition-all duration-300 items-center relative flex"
    >
      <Image
      ...
      />
      <div className=" w-full h-full flex z-20 flex-col gap-5  pb-20 pt-30 px-5 ">
        <p className="text-xs">
          {` ${client.opinion}`}
        </p>
        <h3>{client.client}</h3>
